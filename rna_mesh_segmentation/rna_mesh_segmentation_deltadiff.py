...
pretrain_path = os.path.join(base_path, "pretrained_models/rna_mesh_seg_deltadiff_{}_4x128.pth".format(input_features))
model_save_path = os.path.join(base_path, "data/saved_models/rna_mesh_seg_deltadiff_{}_4x128.pth".format(input_features))
...
def train_epoch(epoch):
    ...
    for data in tqdm(train_loader):
        ...
        # Apply the model
        if use_deltaconv:
            preds = model(features, mass, pos=verts, L=L, evals=evals, evecs=evecs, gradX=gradX, gradY=gradY, faces=faces)
        else:
